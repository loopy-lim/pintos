 ...
 * 여기에는 VM_UNINIT 타입만 분류가 되어있는데 모든 타입을 여기서 분류를 해야한다.
 * 왜?
*/
...
초기화되지 않은 페이지의 swap_in 핸들러는 해당 타입에 따라 페이지를 자동으로 초기화하고, 
주어진 AUX로 INIT를 호출한다. 페이지 구조체를 얻은 후에는, 
이 페이지를 프로세스의 보조 페이지 테이블에 삽입해라. 
vm.h에 정의된 VM_TYPE 매크로를 사용하면 편리할 수 있다.

페이지 폴트 핸들러는 호출 체인을 따라가다가 결국 swap_in을 호출할 때 uninit_intialize 에 도달한다. 
Pintos는 이에 대한 완전한 구현을 제공한다. 
하지만 우리의 설계에 따라 uninit_intialize 를 수정해야 할 수도 있다.
 */
...
/* 현재 페이지의 spt를 가지고 와서 지금 할당하려는 페이지가 spt에 있는지 확인한다.
 * 없다면 새페이지를 할당하고 spt를 업데이트 해야한다.
 */
...
/* 제거될 페이지를 victim이라고 부른다. 프레임이 가득 찼을때 제거할 페이지를 선택하는건가? */
static struct frame *vm_get_victim(void) {
...
bool vm_claim_page(void *va) {
  struct page *page = NULL;
  /* TODO: Fill this function */
  // claim 페이지를 여기서 선별을 하는 듯 하다. 그리고 그 페이지를 claim 할 수 있게 do_claim_page로 넘긴다.
